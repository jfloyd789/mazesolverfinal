...
public class Maze {
   ...
   private void createMaze(String filename) {
       try {
           ...
           // The row and col are specified in the first line of the file im readig in
           ...
           for (int row=0; row<this.numRows; row++) {
               ...
               for (int col=0; col<this.numCols; col++) {
                   // Create a new MazeCell for each location that i need. 
   ...
   /**
    * Prints the maze with solution 

    * @param sol The arraylist of maze cells that make up the answer. 
    */
    ...
    */
    public boolean isValidCell(int row, int col) {
        if (row >= mazeGrid.length || row < 0 || col >= mazeGrid[0].length || col < 0) {
            return false;
        }
        MazeCell cel = mazeGrid[row][col];
        if (cel.isExplored()) {
            return false;
        }
        if (cel.isWall()) {
